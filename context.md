...
> Use this file as context when continuing development in a new Enchanté chat window.
> Project location: `/Users/nandha_handharu/Documents/Nandha/Github/pickleball`
> Preview file: `/Users/nandha_handharu/Documents/Enchanté/Conversations/C6E0DFCE-C8D1-4ED6-8B5B-969137F66863/pickleball-preview.html`
...
| Hosting | Fly.io (Singapore region `sin`) |
...
- URL: `https://pickleball-booking.fly.dev`
- Fly.io volume: `pickleball_data` mounted at `/data` (stores `pickleball.db`)
...
- Default password: `pickle2024` (override via `flyctl secrets set ADMIN_PASSWORD=xxx`)
- Primary: `express-session` (in-memory store)
- Fallback: `X-Admin-Token` header = `btoa(password)` — survives fly.io machine restarts
...
### Singles (1v1)
Greedy: cover all C(N,2) pairs, 1 per game.
...
4. **Fly.io sessions**: In-memory sessions clear on auto-stop. `X-Admin-Token` header fallback handles this.
...
7. **SQLite volume**: Database on persistent volume `/data`. Survives deploys, NOT machine deletion.
